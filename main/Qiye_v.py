...
def qiye_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __hasMessage__=qiye.__hasMessage__
        except:
            __hasMessage__=None
        if  __hasMessage__=="是":
            tablename=request.session.get("tablename")
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'qiye' != 'forum':
            if req_dict.get("userid") and 'qiye' != 'chat':
                del req_dict["userid"]
        else:
            #非管理员权限的表,判断当前表字段名是否有userid
            if tablename!="users" and 'qiye'[:7]!='discuss'and "userid" in qiye.getallcolumn(qiye,qiye):
                req_dict["userid"] = request.session.get("params").get("id")
...
def qiye_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if qiye.objects.filter(qiyezhanghao = req_dict['qiyezhanghao']).count()>0:
            ...
            msg['msg'] = "企业账号已存在"
            return JsonResponse(msg)
        if qiye.objects.filter(qiyemingcheng = req_dict['qiyemingcheng']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "企业名称已存在"
            return JsonResponse(msg)
        tablename=request.session.get("tablename")
...
def qiye_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if qiye.objects.filter(id__gt=req_dict['id'],qiyezhanghao = req_dict['qiyezhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "企业账号已存在"
            return JsonResponse(msg)
        if qiye.objects.filter(id__gt=req_dict['id'],qiyemingcheng = req_dict['qiyemingcheng']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "企业名称已存在"
            return JsonResponse(msg)
...
# （按值统计）时间统计类型
def qiye_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
...
# 按值统计
def qiye_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM qiye {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
# （按值统计）时间统计类型(多)
def qiye_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM qiye {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
# （按值统计(多)）
def qiye_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, sum({1}) AS total FROM qiye {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def qiye_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM qiye " + where + " GROUP BY " + columnName
