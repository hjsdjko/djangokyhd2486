...
def mianshiyaoqing_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        records=mianshiyaoqing.getbyparams(mianshiyaoqing, mianshiyaoqing, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)
...
def mianshiyaoqing_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'mianshiyaoqing' != 'forum':
            if req_dict.get("userid") and 'mianshiyaoqing' != 'chat':
                del req_dict["userid"]
        else:
        ...
        msg['data']['pageSize']  =mianshiyaoqing.page(mianshiyaoqing, mianshiyaoqing, req_dict, request, q)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in mianshiyaoqing.getallcolumn(mianshiyaoqing,mianshiyaoqing):
...
#分类列表
def mianshiyaoqing_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = mianshiyaoqing.page(mianshiyaoqing, mianshiyaoqing, {})
        return JsonResponse(msg)
...
def mianshiyaoqing_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'clicktime' in req_dict.keys() and req_dict['clicktime']=="None":
            del req_dict['clicktime']
        if req_dict.get("mima") and "mima" not in mianshiyaoqing.getallcolumn(mianshiyaoqing,mianshiyaoqing) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in mianshiyaoqing.getallcolumn(mianshiyaoqing,mianshiyaoqing) :
...
def mianshiyaoqing_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM mianshiyaoqing {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
# （按值统计）时间统计类型(多)
def mianshiyaoqing_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM mianshiyaoqing {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM mianshiyaoqing {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM mianshiyaoqing {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
# （按值统计(多)）
def mianshiyaoqing_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, sum({1}) AS total FROM mianshiyaoqing {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def mianshiyaoqing_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM mianshiyaoqing " + where + " GROUP BY " + columnName
