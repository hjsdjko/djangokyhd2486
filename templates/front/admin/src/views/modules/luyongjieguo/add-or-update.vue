	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"0px 30px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="录用结果" prop="luyongjieguo">
					<el-select :disabled="ro.luyongjieguo" v-model="ruleForm.luyongjieguo" placeholder="请选择录用结果" >
						<el-option
							v-for="(item,index) in luyongjieguoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="录用结果" prop="luyongjieguo">
					<el-input v-model="ruleForm.luyongjieguo"
						placeholder="录用结果" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="录用岗位" prop="zhiweileixing">
					<el-input v-model="ruleForm.zhiweileixing" placeholder="录用岗位" clearable  :readonly="ro.zhiweileixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="录用岗位" prop="zhiweileixing">
					<el-input v-model="ruleForm.zhiweileixing" placeholder="录用岗位" readonly></el-input>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='luyongjieguo'){
							this.ruleForm.luyongjieguo = obj[o];
							this.ro.luyongjieguo = true;
							continue;
						}
						if(o=='zhiweileixing'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `luyongjieguo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.luyongjieguoCrossAddOrUpdateFlag = false;
						 this.parent.search();
