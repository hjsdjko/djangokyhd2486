...
<!-- 商品推荐 -->
<div id="animate_recommendzhiweizhaopin" class="recommend animate__animated" :style='{"width":"80%","margin":"0 auto","position":"relative","background":"#fff","order":"2"}'>
    ...
    <div class="title" :style='{"margin":"10px 0","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231122/a500d6f740e5474287d3831acdce1d92.png)","width":"30%","lineHeight":"54px","backgroundSize":"contain","backgroundPosition":"left top","backgroundRepeat":"no-repeat"}'>
		<span :style='{"color":"#000","fontSize":"32px","fontWeight":"bold"}'>职位招聘推荐</span>
	</div>
	...
	<div v-if="zhiweizhaopinRecommend.length" class="list list12 index-pv1 list12zhiweizhaopin" :style='{"width":"100%","padding":"10px","background":"#fff","display":"flex","height":"auto"}'>
	  <div class="tab" :style='{"padding":"0","boxShadow":" 0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"0 0 0 20px","flexWrap":"wrap","background":"#FFFBFB","display":"flex","width":"170px","height":"auto","order":"2"}'>
	    <template v-for="item,index in recommendList12zhiweizhaopin">
		<div v-if="index < 6" @click="recommendIndexClick12(index + 1, 'zhiweizhaopin')" class="item" :class="index + 1 == recommendIndex12zhiweizhaopin ? 'active' : ''">
			...
			<div :style='{"color":"inherit","fontSize":"14px"}'>{{item[recommendColumn12zhiweizhaopin]}}</div>
		</div>
		</template>
		<div class="more" @click="moreBtn('zhiweizhaopin')">
	  ...
	  <div class="list" :style='{"overflow":"hidden","flexWrap":"wrap","flex":"1","background":"#fff","display":"flex","justifyContent":"space-between","height":"auto","order":"1"}'>
	    <div :style='{"border":"1px solid #D8D8D8","width":"calc(100% / 3 - 20px)","margin":"0 0 20px","position":"relative","borderWidth":"0 0 2px","height":"auto"}' class="item list-item animation-box" v-for="item,index in zhiweizhaopinRecommend" :key="index" @click="toDetail('zhiweizhaopinDetail', item)">
			<img :name="item.id" :style='{"width":"100%","objectFit":"cover","display":"block","height":"200px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
			<img :name="item.id" :style='{"width":"100%","objectFit":"cover","display":"block","height":"200px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
			<div class="line1" :style='{"padding":"0 10px","lineHeight":"32px","fontSize":"18px","color":"#000","textAlign":"center"}'>{{item.qiyemingcheng}}</div>
			<div class="line1" :style='{"padding":"0 10px","lineHeight":"32px","fontSize":"18px","color":"#000","textAlign":"center"}'>{{item.zhiweileixing}}</div>
			<div class="line1" :style='{"padding":"0 10px","lineHeight":"32px","fontSize":"18px","color":"#000","textAlign":"center"}'>发布时间:{{item.fabushijian}}</div>
	    </div>
	  </div>
	</div>
	...
	<div @click="moreBtn('zhiweizhaopin')" :style='{"border":"1px solid #9E9E9E","margin":"0 0 0 -40px","top":"22px","textAlign":"center","left":"33%","background":"none","display":"block","width":"80px","lineHeight":"32px","position":"absolute"}'>
...
<!-- 特价商品 -->
<div id="animate_listqiye" class="lists animate__animated" :style='{"padding":"0 0 100px","margin":"0 0 0","background":"url(http://codegen.caihongy.cn/20231122/7eef55d4e07f4b0598a9e072cb72541b.png)","backgroundSize":"cover","backgroundPosition":"center center","position":"relative","backgroundRepeat":"no-repeat","order":"7"}'>
	...
	<div class="title" :style='{"width":"30%","margin":"30px 10%","lineHeight":"54px","textAlign":"left","background":"none"}'>
	  <span :style='{"color":"#000","fontSize":"32px","fontWeight":"bold"}'>企业展示</span>
	</div>
	...
	<div class="list list15 index-pv1" :style='{"padding":"0","margin":"0 auto","alignItems":"center","display":"flex","width":"80%","justifyContent":"center","height":"auto"}'>
	  <div :style='{"width":"32%","height":"auto"}' class="list left">
		<template v-if="qiyeList.length">
	    <template v-for="item,index in qiyeList">
		<div v-if="index == 0" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0 0 30px","position":"relative","background":"#fff","height":"auto"}' class="box1 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
		  <div class="info">
	      </div>
	    </div>
		</template>
		</template>
		<template v-if="qiyeList.length > 1">
		<template v-for="item,index in qiyeList">
	    <div v-if="index == 1" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0","position":"relative","background":"#fff","height":"auto"}' class="box2 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
		  <div class="info">
	      </div>
	  ...
	  <div :style='{"width":"32%","margin":"0 2%","height":"auto"}' class="list center">
		<template v-if="qiyeList.length > 2">
		<template v-for="item,index in qiyeList">
	    <div v-if="index == 2" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0 0 15px","position":"relative","background":"#fff","height":"auto"}' class="box1 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"200px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"200px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
		  <div class="info">
		  </div>
	    ...
	    <div :style='{"margin":"0 0 15px","alignItems":"center","background":"#fff","flexDirection":"column","display":"flex","width":"100%","justifyContent":"center","height":"200px"}' class="box2">
	      <div :style='{"color":"#4D4D4D","fontSize":"16px","lineHeight":"1.5"}' class="desc">{{qiyeList[2].shhf}}</div>
	    </div>
		<template v-if="qiyeList.length > 3">
		<template v-for="item,index in qiyeList">
	    <div v-if="index == 3" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0","position":"relative","background":"#fff","height":"auto"}' class="box3 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"200px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"200px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
		  <div class="info">
		  </div>
	  ...
	  <div :style='{"width":"32%","height":"auto"}' class="list right">
	    <template v-if="qiyeList.length > 4">
	    <template v-for="item,index in qiyeList">
	    <div v-if="index == 4" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0 0 30px","position":"relative","background":"#fff","height":"auto"}' class="box1 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
		  <div class="info">
	      </div>
		...
		<template v-if="qiyeList.length > 5">
		<template v-for="item,index in qiyeList">
	    <div v-if="index == 5" @click="toDetail('qiyeDetail', item)" :style='{"width":"100%","margin":"0","position":"relative","background":"#fff","height":"auto"}' class="box2 list-item animation-box">
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-if="preHttp(item.qiyetupian)" :src="item.qiyetupian.split(',')[0]" alt="" />
		  <img :name="item.id" :style='{"width":"100% ","objectFit":"cover","display":"block","height":"300px"}' v-else :src="baseUrl + (item.qiyetupian?item.qiyetupian.split(',')[0]:'')" alt="" />
	      <div class="info">
	      </div>
	...
	<div @click="moreBtn('qiye')" :style='{"border":"1px solid #000","margin":"10px auto","top":"30px","textAlign":"center","background":"none","display":"block","width":"80px","lineHeight":"32px","position":"absolute","right":"10%"}'>
  ...
  export default {
    ...
    data() {
      return {
        ...
        newsList: [],
        zhiweizhaopinRecommend: [],

        qiyeList: [],

		newListSwiper13news: null,

		recommendIndex12zhiweizhaopin: 0,
		recommendList12zhiweizhaopin: [],
		recommendColumn12zhiweizhaopin: '',
    ...
    created() {
		...
		this.getList();
		this.zhiweizhaopinRecommendCate()
    },
	mounted() {
    ...
    methods: {
		...
		zhiweizhaopinRecommendCate(){
			this.$http.get('zhiweileixing/list',{}).then(res => {
			  if (res.data.code == 0) {
				this.recommendList12zhiweizhaopin = res.data.data.list
			  }
			});
			this.recommendColumn12zhiweizhaopin = 'zhiweileixing'
		},
		...
		handleScroll() {
			let arr = [
				...
				{id:'animate_listqiye',css:'animate__flash'},
				{id:'animate_recommendzhiweizhaopin',css:'animate__rubberBand'},
      ...
      getSystemIntroduction() {
		...
		getList() {
			let autoSortUrl = "";
			let data = {}
          autoSortUrl = "zhiweizhaopin/autoSort";
			data = {
				page: 1,
				limit: 6,
			}
			if(this.recommendIndex12zhiweizhaopin != 0){
				data.zhiweileixing = this.recommendList12zhiweizhaopin[this.recommendIndex12zhiweizhaopin - 1].zhiweileixing
			}
			this.$http.get(autoSortUrl, {params: data}).then(res => {
				if (res.data.code == 0) {
					this.zhiweizhaopinRecommend = res.data.data.list;
